...
process.once('cleanup', () => {
  if (client) {
    client.addListener('exit', () => process.exit());
    client.kill('SIGTERM');
...
// Launch `node build/client.js` on a background thread
function spawnClient() {
  return cp.spawn('node',
    [
      ...
      ...(isDebug ? ['./client.js'] : [
        '--eval',
        'process.stdin.on("data", data => { if (data.toString() === "load") require("./client.js"); });',
      ]),
    ],
  ...
  .then(() => build({
    ...
    onComplete() {
      ...
      if (isDebug) {
        ...
        client = serverQueue.splice(0, 1)[0] || spawnClient();
        client.stdin.write('load'); // this works faster than IPC
        while (serverQueue.length < 3) serverQueue.push(spawnClient());
      }
    },
  }))
